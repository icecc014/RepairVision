...
async function load() {
  loading.value = true
  try {
    const all = await apiAdminUsers({ role: filter.role || 0, keyword: filter.keyword })
    // 状态筛选在本地完成：接口把 status=0（停用）当作“未传”，无法直接筛停用
    list.value = filter.status === 0 || filter.status === 1 ? all.filter((u) => u.status === filter.status) : all
  } catch (err) {
    ElMessage.error((err as Error).message)
  } finally {
    loading.value = false
