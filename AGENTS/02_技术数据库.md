...
| MySQL驱动 | github.com/go-sql-driver/mysql v1.8.1 | go-zero sqlx 内置依赖，P1落库使用 |
| 密码哈希 | golang.org/x/crypto v0.26.0 | bcrypt，种子账号密码哈希 |
| JWT库 | github.com/golang-jwt/jwt/v4 v4.5.0 | 登录签发（go-zero 间接转直接） |

## 2. 环境变量模板（.env.example）
```bash
# MySQL（单实例承载三个独立库，三服务使用 root + MYSQL_ROOT_PASSWORD 连接各自 schema）
MYSQL_ROOT_PASSWORD=change-me-on-first-run

# JWT
JWT_SECRET=please-replace-with-a-random-string-at-least-32-chars
...
DB:
  DataSource: "root:${MYSQL_ROOT_PASSWORD}@tcp(mysql:3306)/order_db?charset=utf8mb4&parseTime=true&loc=Local"
